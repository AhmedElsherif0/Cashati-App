...
class GoalsRepoImpl with MixinGoals implements GoalsRepository {
  ...
  Future<void> addGoal({required GoalModel goalModel}) async {
    if (isEqualToday(date: goalModel.goalStartSavingDate)) {
      print(
          'is equal today in if ?${isEqualToday(
              date: goalModel.goalStartSavingDate)}');
      // await allExpensesModel.add(expenseModel);
      await goalBox.put(goalModel.id, goalModel);
      await addGoalToRepeatedBox(goalModel);
      print(
          "name of the value added by  key is ${goalBox.get(goalModel.id)!
              .goalName} and key is ${goalBox.get(goalModel.id)!
              .id} \n and Date of today is ${today} and date of choosen");
    } else {
      print(
          'is  equal today in else  ?${isEqualToday(
              date: goalModel.goalStartSavingDate)}');
      await addGoalToRepeatedBox(goalModel);
    }
  ...
  Future<void> deleteGoalFromGoalsBox(GoalModel goalModel) async {
    try {
      ...
      await goalRepeatedBox.get(goalModel.id)?.delete();
    } catch (e) {
  ...
  Future<void> yesConfirmGoal({required GoalModel goalModel,num? newAmount})async {
    ...
    try {
      if (goalModel.goalSaveAmountRepeat == AppStrings.daily) {
        GoalRepeatedDetailsModel theEditedDailyGoal =
        editDailyGoalLastShown(goalModel: goalModel);
        await saveDailyGoalAndAddToRepeatBox(theMatchingGoalinRep:  theEditedDailyGoal,newAmount: newAmount);
        // print('After Edit Daily ${theMatchingDailyExpense.lastConfirmationDate}');
      }
      if (goalModel.goalSaveAmountRepeat == AppStrings.weekly) {
        GoalRepeatedDetailsModel theEditedWeeklyGoal =
        editWeeklyGoalLastShown(
            goalModel: goalModel, today: today);
        await saveWeeklyGoalAndAddToRepeatBox(theMatchingWeeklyExpenseModel: theEditedWeeklyGoal,newAmount: newAmount);
      }
      if (goalModel.goalSaveAmountRepeat ==AppStrings.monthly) {
        GoalRepeatedDetailsModel theEditedMonthlyGoal =
        editMonthlyGoalLastShown(
            goalModel:  goalModel, today: today);
        saveMonthlyGoalAndAddToRepeatBox(theMatchingMonthlyExpenseModel: theEditedMonthlyGoal,NewAmount:newAmount );
      }
    } catch (error) {
